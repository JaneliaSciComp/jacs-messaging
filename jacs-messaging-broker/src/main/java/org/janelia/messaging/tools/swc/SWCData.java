...
public class SWCData {
    ...
    public SWCData(List<SWCNode> nodeList, List<String> headerList, double[] neuronCenter) {
        this.nodeList = nodeList;
        this.headerList = headerList;
        this.neuronCenter = neuronCenter;
    }
    ...
    public void write(File swcFile, int offset) throws Exception {
        if (isValid()) {
            if (offset != -1) {
                // we're exporting a bunch of swc to a dir, and the dir is named like the
                //  given "all neurons" filename; if the original neuron has the expected
                //  .swc extension, remove it and use that for the dir name; if not, append
                //  "dir" to the name instead
                String swcFileName = swcFile.getName();
                String parentDirName;
                if (swcFileName.endsWith(STD_SWC_EXTENSION)) {
                    parentDirName = swcFileName.substring(0, swcFileName.length() - STD_SWC_EXTENSION.length());
                } else {
                    parentDirName = swcFileName + "-dir";
                }
                ...
                if (parseName() != "") {
                    ...
                    String newName = NamingUtils.getIteratedName(swcFileName, offset);
    ...
    /**
     * return the file path of the swcfile; after writing the swc,
     * this will contain the actual file path, post-deconflicting
     */
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(infile))) {
            ...
            while (null != (inline = br.readLine())) {
                if (inline.startsWith("#")) {
                    ...
                    int afterDigits = NamingUtils.findFirstNonDigitPosition(inline);
            ...
            if (newFileText.size() <= 1) {
                ...
                for (StringBuilder builder : newFileText) {
                    File newFile = new File(tempDir, NamingUtils.getIteratedName(infile.getName(), fileNum));
    ...
    protected String reorder(String inline, int rootOffset) {
        ...
        int lastDigitPos = NamingUtils.lastDigitPosition(inline);
        int afterDigitsPos = NamingUtils.findFirstNonDigitPosition(inline);
